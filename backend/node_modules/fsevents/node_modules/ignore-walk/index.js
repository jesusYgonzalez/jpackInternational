...
class Walker extends EE {
  ...
  emit(ev, data) {
    ...
    if (!(this.sawError && ev === "error")) {
      if (ev === "error") this.sawError = true;
      else if (ev === "done" && !this.parent) data = data.sort(this.sort);
      if (ev === "error" && this.parent) ret = this.parent.emit("error", data);
      else ret = super.emit(ev, data);
    }
    return ret;
  }
  ...
  onReaddir(entries) {
    ...
    if (entries.length === 0) {
      ...
      if (hasIg) this.addIgnoreFiles();
      else this.filterEntries();
    }
  }
  ...
  stat(entry, file, dir, then) {
    ...
    fs[this.follow ? "stat" : "lstat"](abs, (er, st) => {
      ...
      else this.onstat(st, entry, file, dir, then);
    });
  }
        ...
        this.ignoreRules[f].forEach(rule => {
          ...
          if (rule.negate !== included) {
            ...
            const match =
              ...
              rule.match(entry) ||
              (!!partial &&
                (rule.match("/" + entry + "/") || rule.match(entry + "/"))) ||
              (!!partial &&
                rule.negate &&
