...
class Unpack extends Parser {
  ...
  [CHECKPATH](entry) {
    ...
    if (this.win32) {
      ...
      entry.path =
        parsed.root === ""
          ? wc.encode(entry.path)
          : parsed.root + wc.encode(entry.path.substr(parsed.root.length));
    }

    if (path.isAbsolute(entry.path)) entry.absolute = entry.path;
    else entry.absolute = path.resolve(this.cwd, entry.path);
    ...
    this[MKDIR](path.dirname(entry.absolute), this.dmode, er => {
      ...
      fs.lstat(entry.absolute, (er, st) => {
        if (st && (this.keep || (this.newer && st.mtime > entry.mtime)))
          this[SKIP](entry);
        else if (er || this[ISREUSABLE](entry, st)) this[MAKEFS](null, entry);
        else if (st.isDirectory()) {
          if (entry.type === "Directory") {
            if (!entry.mode || (st.mode & 0o7777) === entry.mode)
              this[MAKEFS](null, entry);
            else
              fs.chmod(entry.absolute, entry.mode, er =>
...
class UnpackSync extends Unpack {
  ...
  [CHECKFS](entry) {
    ...
    try {
      ...
      else if (this[ISREUSABLE](entry, st)) return this[MAKEFS](null, entry);
      else {
        try {
          if (st.isDirectory()) {
            if (entry.type === "Directory") {
