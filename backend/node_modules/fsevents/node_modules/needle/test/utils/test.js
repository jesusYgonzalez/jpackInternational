...
function multipart_post(url) {
  ...
  var black_pixel = Buffer.from(
    "data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=".replace(
      /^data:image\/\w+;base64,/,
      ""
    ),
    "base64"
  ...
  var data = {
    ...
    field2: {
      value: JSON.stringify({ json: [{ one: 1 }, { two: 2 }] }),
...
switch (process.argv[2]) {
  ...
  case "multipart":
    multipart_post(
      process.argv[3] || "http://posttestserver.com/post.php?dir=example"
    );
    break;
  case "all":
    simple_head();
