...
try {
  ...
  var ownPackageJSON = fs.readFileSync(
  ...
  download(from, opts, function(err, req) {
    ...
    req.on("close", function() {
      if (!hasResponse) {
        hasResponse = true;
        return callback(
          new Error("Connection closed while downloading tarball file")
        );
      }
    });
    ...
    req.on("response", function(res) {
      ...
      if (res.statusCode !== 200) {
        badDownload = true;
        var err = new Error(
          res.statusCode + " status code downloading tarball " + from
        );
...
function install(gyp, argv, callback) {
  ...
  if (should_do_source_build) {
    ...
    var fallback_to_build =
      gyp.opts["fallback-to-build"] || gyp.opts.fallback_to_build;
    var should_do_fallback_build =
      fallback_to_build === package_json.name ||
      (fallback_to_build === true || fallback_to_build === "true");
