...
module.exports = function castUpdate(schema, obj, options, context, filter) {
  ...
  while (i--) {
    ...
    if (
      ...
      const msg =
        "Invalid atomic update value for " +
        op +
        ". " +
...
function walkUpdatePath(schema, obj, op, options, context, filter, pref) {
  ...
  while (i--) {
    ...
    if (val && val.constructor.name === "Object") {
      ...
      if (schematype && schematype.caster && op in castOps) {
        ...
        if ("$each" in val) {
          ...
          try {
            ...
            aggregatedError = _handleCastError(
              error,
              context,
              key,
              aggregatedError
      ...
      if (schematype == null) {
        const _res = getEmbeddedDiscriminatorPath(
          schema,
          obj,
          filter,
          checkPath
      ...
      const skip =
        isStrict && !schematype && !/real|nested/.test(pathDetails.pathType);
...
function castUpdateVal(schema, val, op, $conditional, context, path) {
  ...
  if (overwriteOps[op]) {
    return schema.castForQueryWrapper({
      val: val,
      context: context,
      $skipQueryCastForUpdate:
        val != null && schema.$isMongooseArray && schema.$parentSchema
    });
  }
