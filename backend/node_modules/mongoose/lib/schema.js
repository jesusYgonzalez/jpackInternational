...
const hookNames = queryHooks
  .concat(documentHooks)
...
Schema.prototype.add = function add(obj, prefix) {
  ...
  for (let i = 0; i < keys.length; ++i) {
    ...
    if (
      ...
      throw new TypeError(
        "Invalid value for schema Array path `" + prefix + key + "`"
      );
    }

    if (
      utils.isPOJO(obj[key]) &&
      (!obj[key][this.options.typeKey] ||
        (this.options.typeKey === "type" && obj[key].type.type))
    ) {
...
const warnings = {};
warnings.increment =
  "`increment` should not be used as a schema path name " +
...
Schema.prototype.path = function(path, obj) {
  ...
  if (schemaType.$isSchemaMap) {
    ...
    const mapPath = path + ".$*";
    this.paths[path + ".$*"] = this.interpretAsType(
      mapPath,
      obj.of || { type: {} },
      this.options
    );
...
Schema.prototype.interpretAsType = function(path, obj, options) {
  ...
  if (Array.isArray(type) || Array === type || type === "array") {
    ...
    if (
      ...
      return new MongooseTypes.DocumentArray(
        path,
        cast[options.typeKey],
        obj,
        cast
    ...
    if (Array.isArray(cast)) {
      return new MongooseTypes.Array(
        path,
        this.interpretAsType(path, cast, options),
        obj
      );
    }
    ...
    if (typeof cast === "string") {
      ...
      cast &&
      (!cast[options.typeKey] ||
        (options.typeKey === "type" && cast.type.type)) &&
      utils.isPOJO(cast)
    ) {
...
Schema.prototype.virtual = function(name, options) {
    ...
    return virtual
      ...
      .set(function(_v) {
        ...
        if (options.justOne || options.count) {
          ...
          this.$$populatedVirtuals[name] = this.$$populatedVirtuals[
            name
          ].filter(function(doc) {
  ...
  virtuals[name] = parts.reduce(function(mem, part, i) {
    mem[part] ||
      (mem[part] =
        i === parts.length - 1 ? new VirtualType(options, name) : {});
    return mem[part];
  }, this.tree);
...
function isArrayFilter(piece) {
  return (
    piece.indexOf("$[") === 0 && piece.lastIndexOf("]") === piece.length - 1
  );
}
