...
function Document(obj, fields, skipId, options) {
  ...
  const hasIncludedChildren =
    exclude === false && fields ? $__hasIncludedChildren(fields) : {};
...
Document.prototype.$__buildDoc = function(
  obj,
  fields,
  skipId,
  exclude,
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  function _handleIndex(i) {
    ...
    if (
      ...
      if (pathtype === "real" || pathtype === "virtual") {
        ...
        this.$set(
          prefix + key,
          path[key].toObject({ transform: false }),
          constructing
        );
  ...
  try {
    ...
    const refMatches = (() => {
      ...
      if (
        ref != null &&
        (ref === model.modelName || ref === model.baseModelName)
      ) {
        return true;
      }
    ...
    if (
      ...
      (schema.options[this.schema.options.typeKey][0].ref ===
        val[0].constructor.baseModelName ||
        schema.options[this.schema.options.typeKey][0].ref ===
          val[0].constructor.modelName)
    ) {
      if (this.ownerDocument) {
...
Document.prototype.get = function(path, type, options) {
  ...
  if (schema != null && options.getters !== false) {
    ...
    return applyGetters(this, utils.clone(obj) || {}, "virtuals", {
      path: path
    });
  }
...
function _getPathsToValidate(doc) {
  ...
  let paths = Object.keys(doc.$__.activePaths.states.require).filter(function(
    path
  ) {
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (
      ...
      (_pathType.$isMongooseDocumentArray &&
        !get(_pathType, "schemaOptions.required"))
    ) {
      continue;
    }
...
Document.prototype.$__validate = function(options, callback) {
  ...
  const _complete = () => {
    ...
    if (err) {
      for (const key in err.errors) {
        ...
        if (
          !this[documentArrayParent] &&
          err.errors[key] instanceof MongooseError.CastError
        ) {
  ...
  const validatePath = function(path) {
    ...
    process.nextTick(function() {
      ...
      const scope =
        path in _this.$__.pathsToScopes ? _this.$__.pathsToScopes[path] : _this;
      ...
      p.doValidate(
        ...
        function(err) {
          if (
            err &&
            (!p.$isMongooseDocumentArray || err.$isArrayValidatorError)
          ) {
            if (
              p.$isSingleNested &&
...
Document.prototype.$__reset = function reset() {
  ...
  this.$__.activePaths
    ...
    .filter(function(val) {
      return (
        val && val instanceof Array && val.isMongooseDocumentArray && val.length
      );
    })
...
Document.prototype.$__handleReject = function handleReject(err) {
  ...
  if (this.listeners("error").length) {
    ...
    this.constructor.listeners("error").length
  ) {
...
Document.prototype.$toObject = function(options, json) {
  ...
  defaultOptions = utils.options(
    defaultOptions,
    clone(schemaOptions[path] || {})
  );
  ...
  const depopulate =
    options.depopulate || get(options, "_parentOptions.depopulate", false);
...
Document.prototype.depopulate = function(path) {
  ...
  let populatedIds;
  const virtualKeys = this.$$populatedVirtuals
    ? Object.keys(this.$$populatedVirtuals)
    : [];
