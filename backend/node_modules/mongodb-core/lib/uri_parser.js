...
function parseSrvConnectionString(uri, options, callback) {
  ...
  if (result.hostname.split(".").length < 3) {
    return callback(
      new MongoParseError("URI does not have hostname, domain name and tld")
    );
  }
  ...
  if (result.pathname && result.pathname.match(",")) {
    return callback(
      new MongoParseError("Invalid URI, cannot contain multiple hostnames")
    );
  }
  ...
  if (result.port) {
    return callback(
      new MongoParseError(
        `Ports not accepted with '${PROTOCOL_MONGODB_SRV}' URIs`
      )
    );
  }
  ...
  dns.resolveSrv(`_mongodb._tcp.${lookupAddress}`, (err, addresses) => {
    ...
    result.host = addresses
      .map(address => `${address.name}:${address.port}`)
      .join(",");
      ...
      if (record) {
        ...
        if (
          Object.keys(record).some(
            key => key !== "authSource" && key !== "replicaSet"
          )
        ) {
          return callback(
            new MongoParseError(
              "Text record must only set `authSource` or `replicaSet`"
            )
          );
        }
...
function applyAuthExpectations(parsed) {
  ...
  if (authMechanism != null) {
    if (
      ...
      throw new MongoParseError(
        `Username required for mechanism \`${authMechanism}\``
      );
    }
    ...
    if (authMechanism === "MONGODB-X509") {
      if (parsed.auth && parsed.auth.password != null) {
        throw new MongoParseError(
          `Password not allowed for mechanism \`${authMechanism}\``
        );
      }
...
function parseConnectionString(uri, options, callback) {
  ...
  if (authorityParts.length > 2) {
    return callback(
      new MongoParseError("Unescaped at-sign in authority section")
    );
  }
  ...
  if (authorityParts.length > 1) {
    ...
    if (authParts.length > 2) {
      return callback(
        new MongoParseError("Unescaped colon in authority section")
      );
    }
  ...
  const hosts = authorityParts
    ...
    .map(host => {
      ...
      if (parsedHost.path === "/:") {
        hostParsingError = new MongoParseError(
      ...
      if (Number.isNaN(parsedHost.port)) {
        hostParsingError = new MongoParseError(
          "Invalid port (non-numeric string)"
        );
        return;
      }
      ...
      if (result.port > 65535) {
        hostParsingError = new MongoParseError(
          "Invalid port (larger than 65535) with hostname"
        );
        return;
      }
