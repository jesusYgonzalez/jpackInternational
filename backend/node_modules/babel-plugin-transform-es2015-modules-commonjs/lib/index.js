  ...
  var reassignmentVisitor = {
    ReferencedIdentifier: function ReferencedIdentifier(path) {
      ...
      if (path.parentPath.isCallExpression({ callee: path.node })) {
        path.replaceWith(
          t.sequenceExpression([t.numericLiteral(0), replacement])
        );
    ...
    AssignmentExpression: function AssignmentExpression(path) {
      ...
      if (left.isIdentifier()) {
        ...
        for (
          var _iterator2 = left.node.properties,
          ...
          path.insertAfter(
            buildExportsAssignment(t.identifier(_name), t.identifier(_name))
          );
        }
      } else if (left.isArrayPattern()) {
        for (
          var _iterator3 = left.node.elements,
          ...
          path.insertAfter(
            buildExportsAssignment(t.identifier(_name2), t.identifier(_name2))
          );
        }
      }
    },
    UpdateExpression: function UpdateExpression(path) {
      ...
      var node = t.assignmentExpression(
      ...
      Program: {
        exit: function exit(path) {
          ...
          function addRequire(source, blockHoist) {
            ...
            var ref = path.scope.generateUidIdentifier(
              (0, _path2.basename)(source, (0, _path2.extname)(source))
            );
          ...
          for (
            ...
            if (_path.isExportDeclaration()) {
              ...
              var specifiers = [].concat(
                _path.get("declaration"),
              ...
              for (
                ...
                if (ids.__esModule) {
                  throw _specifier2.buildCodeFrameError(
            ...
            if (_path.isImportDeclaration()) {
              ...
              (_importsEntry$specifi = importsEntry.specifiers).push.apply(
                _importsEntry$specifi,
                _path.node.specifiers
              );

              if (typeof _path.node._blockHoist === "number") {
                importsEntry.maxBlockHoist = Math.max(
                  _path.node._blockHoist,
              ...
              if (declaration.isFunctionDeclaration()) {
                ...
                if (_id) {
                  ...
                  _path.replaceWithMultiple([
                    declaration.node,
                    buildExportsAssignment(_defNode, _id)
                  ]);
                } else {
                  _path.replaceWith(
                    buildExportsAssignment(
              ...
              if (_declaration.node) {
                if (_declaration.isFunctionDeclaration()) {
                  ...
                  _path.replaceWithMultiple([
                    _declaration.node,
                    buildExportsAssignment(_id3, _id3)
                  ]);
                  ...
                  for (
                    ...
                    if (_id4.isIdentifier()) {
                      ...
                      init.replaceWith(
                        buildExportsAssignment(_id4.node, init.node).expression
                      );
              ...
              if (_source) {
                ...
                for (
                  ...
                  if (_specifier3.isExportNamespaceSpecifier()) {
                    ...
                    if (
                      !noInterop &&
                      _specifier3.node.local.name === "default"
                    ) {
                      topNodes.push(
                        buildExportsFrom(
                          t.stringLiteral(_specifier3.node.exported.name),
                          t.memberExpression(
                            t.callExpression(
                              this.addHelper("interopRequireDefault"),
                              [ref]
                            ),
                            _specifier3.node.local
                          )
                        )
                      );
                    } else {
                      topNodes.push(
                        buildExportsFrom(
                          t.stringLiteral(_specifier3.node.exported.name),
                          t.memberExpression(ref, _specifier3.node.local)
                        )
                      );
                    }
                    nonHoistedExportNames[
                      _specifier3.node.exported.name
                    ] = true;
                  }
                }
              } else {
                for (
                  var _iterator9 = _specifiers,
                  ...
                  if (_specifier4.isExportSpecifier()) {
                    addTo(
                      exports,
                      _specifier4.node.local.name,
                      _specifier4.node.exported
                    );
                    nonHoistedExportNames[
                      _specifier4.node.exported.name
                    ] = true;
                    nodes.push(
                      buildExportsAssignment(
                        _specifier4.node.exported,
                        _specifier4.node.local
                      )
                    );
                  }
                }
                ...
                if (t.isImportSpecifier(_specifier)) {
                  ...
                  if (_specifier.imported.name === "default") {
                    if (wildcard) {
                      ...
                      var _varDecl = t.variableDeclaration("var", [
                        t.variableDeclarator(
                          target,
                          t.callExpression(
                            this.addHelper("interopRequireDefault"),
                  ...
                  remaps[_specifier.local.name] = t.memberExpression(
                    t.cloneWithoutLoc(target),
                    t.cloneWithoutLoc(_specifier.imported)
                  );
                }
              }
            } else {
          ...
          if (hasImports && (0, _keys2.default)(nonHoistedExportNames).length) {
            ...
            var nonHoistedExportNamesArr = (0, _keys2.default)(
              nonHoistedExportNames
            );

            var _loop = function _loop(currentExportsNodeAssignmentLength) {
              var nonHoistedExportNamesChunk = nonHoistedExportNamesArr.slice(
            ...
            for (
              var currentExportsNodeAssignmentLength = 0;
              currentExportsNodeAssignmentLength <
...
var _babelPluginTransformStrictMode2 = _interopRequireDefault(
  _babelPluginTransformStrictMode
);
