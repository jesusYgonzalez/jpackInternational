...
var GridStore = function GridStore(db, id, filename, mode, options) {
  deprecationFn();
  if (!(this instanceof GridStore))
    return new GridStore(db, id, filename, mode, options);
  this.db = db;
  ...
  this.root =
    this.options["root"] == null
      ? GridStore.DEFAULT_ROOT_COLLECTION
      : this.options["root"];
  this.position = 0;
  this.readPreference =
    this.options.readPreference ||
    db.options.readPreference ||
  ...
  self.open(function(err, self) {
    ...
    fs.fstat(file, function(err, stats) {
      ...
      var writeChunk = function() {
        ...
        // Read the file
        fs.read(file, _buffer, 0, _buffer.length, offset, function(
          err,
          bytesRead,
          data
        ) {
...
var read = function(self, length, buffer, options, callback) {
  ...
  finalBuffer._index =
    buffer != null && buffer._index != null ? buffer._index : 0;

  if (
    self.currentChunk.length() -
      self.currentChunk.position +
      finalBuffer._index >=
    finalLength
  ) {
    ...
    if (finalLength === 0 && finalBuffer.length === 0)
      return callback(
        MongoError.create({ message: "File does not exist", driver: true }),
        null
      );
    // Else return data
  ...
  // Read the next chunk
  slice = self.currentChunk.readSlice(
    self.currentChunk.length() - self.currentChunk.position
  );
  // Copy content to final buffer
  slice.copy(finalBuffer, finalBuffer._index);
...
var seek = function(self, position, seekLocation, options, callback) {
  ...
  var seekLocationFinal =
    seekLocation == null ? GridStore.IO_SEEK_SET : seekLocation;
...
var _open = function(self, options, callback) {
  ...
  // Create the query
  var query =
    self.referenceBy === REFERENCE_BY_ID
      ? { _id: self.fileId }
  ...
  if (query != null) {
    collection.findOne(query, options, function(err, doc) {
      ...
      if (doc != null) {
        ...
        self.filename =
          self.mode === "r" || self.filename === undefined
            ? doc.filename
            : self.filename;
        ...
        self.length = 0;
        var txtId =
          self.fileId._bsontype === "ObjectID"
            ? self.fileId.toHexString()
            : self.fileId;
        return error(
          MongoError.create({
            message: f(
      ...
      if (self.mode === "r") {
        ...
        deleteChunks(self, options, function(err) {
          ...
          self.currentChunk = new Chunk(self, { n: 0 }, self.writeConcern);
          self.contentType =
            self.options["content_type"] == null
              ? self.contentType
          ...
          self.metadata =
            self.options["metadata"] == null
              ? self.metadata
              : self.options["metadata"];
          self.aliases =
            self.options["aliases"] == null
              ? self.aliases
        ...
        nthChunk(self, lastChunkNumber(self), options, function(err, chunk) {
          ...
          self.currentChunk =
            chunk == null
              ? new Chunk(self, { n: 0 }, self.writeConcern)
              : chunk;
    ...
    if (self.mode === "w") {
      ...
      nthChunk(self, lastChunkNumber(self), options, function(err, chunk) {
        ...
        // Set the current chunk
        self.currentChunk =
          chunk == null ? new Chunk(self, { n: 0 }, self.writeConcern) : chunk;
        self.currentChunk.position = self.currentChunk.data.length();
        self.metadata =
          self.options["metadata"] == null
            ? self.metadata
...
var nthChunk = function(self, chunkNumber, options, callback) {
  ...
  self
    .chunkCollection()
    .findOne({ files_id: self.fileId, n: chunkNumber }, options, function(
      err,
      chunk
    ) {
      if (err) return callback(err);
...
var deleteChunks = function(self, options, callback) {
  ...
  if (self.fileId != null) {
    self
      .chunkCollection()
      .remove({ files_id: self.fileId }, options, function(err) {
...
var exists = function(db, fileIdObject, rootCollection, options, callback) {
  ...
  db.collection(rootCollectionFinal + ".files", function(err, collection) {
    ...
    collection.findOne(query, { readPreference: readPreference }, function(
      err,
      item
    ) {
      if (err) return callback(err);
...
GridStore.list = function(db, rootCollection, options, callback) {
  ...
  return executeOperation(
    db.s.topology,
    list,
    [db, rootCollection, options, callback],
    {
      skipSessions: true
    }
  );
...
var list = function(db, rootCollection, options, callback) {
  ...
  db.collection(rootCollectionFinal + ".files", function(err, collection) {
    ...
    collection.find({}, { readPreference: readPreference }, function(
      err,
      cursor
    ) {
      if (err) return callback(err);
...
var readStatic = function(db, name, length, offset, options, callback) {
  new GridStore(db, name, "r", options).open(function(err, gridStore) {
    ...
    if (offset && length && offset + length > gridStore.length)
      return callback(
        "offset and length is larger than the size of the file",
        null
      );
    ...
    new GridStore(db, names, "w", options).open(function(err, gridStore) {
      ...
      deleteChunks(gridStore, function(err) {
        ...
        gridStore.collection(function(err, collection) {
          if (err) return callback(err);
          collection.remove({ _id: gridStore.fileId }, writeConcern, function(
            err
          ) {
            callback(err, self);
          });
...
var _getWriteConcern = function(self, options) {
  ...
  if (
    finalOptions.w < 1 &&
    (finalOptions.journal === true ||
      finalOptions.j === true ||
      finalOptions.fsync === true)
  )
    throw MongoError.create({
      message:
        "No acknowledgement using w < 1 cannot be combined with journal:true or fsync:true",
      driver: true
    });
...
GridStoreStream.prototype._read = function() {
  ...
  // Set read length
  var length =
    self.gs.length < self.gs.chunkSize
      ? self.gs.length - self.seekPosition
      : self.gs.chunkSize;
  if (!self.gs.isOpen) {
    self.gs.open(function(err) {
      self.totalBytesToRead = self.gs.length - self.gs.position;
