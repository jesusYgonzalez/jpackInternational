...
function addUser(db, username, password, options, callback) {
  ...
  executeAuthCreateUserCommand(db, username, password, options, (err, r) => {
    ...
    if (err && err.code === -5000) {
      ...
      const dbToUse = options.dbName
        ? new Db(options.dbName, db.s.topology, db.s.options)
        : db;
...
function createCollection(db, name, options, callback) {
  ...
  const listCollectionOptions = Object.assign({}, finalOptions, {
    nameOnly: true
  });
      ...
      db.s.topology.insert(
        ...
        (err, result) => {
          ...
          if (result.result.writeErrors)
            return handleCallback(
              callback,
              MongoError.create(result.result.writeErrors[0]),
              null
            );
...
function profilingLevel(db, options, callback) {
  executeCommand(db, { profile: -1 }, options, (err, doc) => {
    if (err == null && doc.ok === 1) {
      ...
      return callback(
        new Error("Error: illegal profiling level value " + was),
        null
      );
    } else {
      err != null
...
function createCreateIndexCommand(db, name, fieldOrSpec, options) {
  ...
  const indexName =
    typeof options.name === "string" ? options.name : indexParameters.name;
  const selector = {
  ...
  const finalUnique =
    options == null || "object" === typeof options ? false : options;
  // Set up options
 ...
 * @param {Db~resultCallback} [callback] The command result callback
 */
function executeAuthCreateUserCommand(
  db,
  username,
  password,
  options,
  ...
  const maxTimeMS =
    typeof options.maxTimeMS === "number" ? options.maxTimeMS : null;
