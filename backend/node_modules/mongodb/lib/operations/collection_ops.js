...
function bulkWrite(coll, operations, options, callback) {
  ...
  if (collation && capabilities && !capabilities.commandsTakeCollation) {
    return callback(
      new MongoError("server/primary/mongos does not support collation")
    );
  }
...
function createIndexes(coll, indexSpecs, options, callback) {
  ...
  for (let i = 0; i < indexSpecs.length; i++) {
    if (indexSpecs[i].name == null) {
      ...
      // Did the user pass in a collation, check if our write server supports it
      if (
        indexSpecs[i].collation &&
        capabilities &&
        !capabilities.commandsTakeCollation
      ) {
        return callback(
          new MongoError("server/primary/mongos does not support collation")
        );
...
function findAndModify(coll, query, sort, doc, options, callback) {
  ...
  if (finalOptions.bypassDocumentValidation === true) {
    queryObject.bypassDocumentValidation =
      finalOptions.bypassDocumentValidation;
  }
...
function findOneAndReplace(coll, filter, replacement, options, callback) {
  ...
  finalOptions.new =
    options.returnOriginal !== void 0 ? !options.returnOriginal : false;
...
function findOneAndUpdate(coll, filter, update, options, callback) {
  ...
  finalOptions.update = true;
  finalOptions.new =
    typeof options.returnOriginal === "boolean"
      ? !options.returnOriginal
      : false;
  finalOptions.upsert =
    typeof options.upsert === "boolean" ? options.upsert : false;
...
function group(
  ...
  if (command) {
    const reduceFunction =
      reduce && reduce._bsontype === "Code" ? reduce : new Code(reduce);
    ...
    if ("function" === typeof keys || (keys && keys._bsontype === "Code")) {
      selector.group.$keyf =
        keys && keys._bsontype === "Code" ? keys : new Code(keys);
    } else {
      const hash = {};
      keys.forEach(key => {
        hash[key] = 1;
    ...
    const scope =
      reduce != null && reduce._bsontype === "Code" ? reduce.scope : {};
...
function optionsOp(coll, opts, callback) {
  coll.s.db
    .listCollections({ name: coll.s.name }, opts)
    .toArray((err, collections) => {
      ...
      if (collections.length === 0) {
        return handleCallback(
          callback,
          MongoError.create({
            message: `collection ${coll.s.namespace} not found`,
            driver: true
          })
        );
      }
...
function parallelCollectionScan(coll, options, callback) {
  ...
  executeCommand(coll.s.db, commandObject, options, (err, result) => {
    ...
    for (let i = 0; i < result.cursors.length; i++) {
      ...
      const cursorId =
        typeof rawId === "number" ? Long.fromNumber(rawId) : rawId;
...
function rename(coll, newName, options, callback) {
  ...
  const dropTarget =
    typeof options.dropTarget === "boolean" ? options.dropTarget : false;
  const cmd = {
...
function replaceOne(coll, filter, doc, options, callback) {
  ...
  updateDocuments(coll, filter, doc, options, (err, r) => {
    ...
    r.modifiedCount =
      r.result.nModified != null ? r.result.nModified : r.result.n;
    r.upsertedId =
