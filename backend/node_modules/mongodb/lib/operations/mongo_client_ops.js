...
function connect(mongoClient, url, options, callback) {
  ...
  const transform = options.useNewUrlParser
    ? transformUrlOptions
    : legacyTransformUrlOptions;
  ...
  parseFn(url, options, (err, _object) => {
    ...
    if (object.servers.length === 0) {
      return callback(
        new Error("connection string must contain at least one seed host")
      );
    }
...
function connectWithUrl(mongoClient, url, options, connectCallback) {
  ...
  if (
    typeof options.readPreference === "string" ||
    typeof options.read_preference === "string"
  ) {
  ...
  const isDoingAuth =
    finalOptions.user || finalOptions.password || finalOptions.authMechanism;
...
function translateOptions(options, translationOptions) {
  ...
  if (
    ...
    (options.readPreferenceTags || options.read_preference_tags)
  ) {
