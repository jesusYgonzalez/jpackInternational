...
module.exports = function(url, options, callback) {
  ...
  if (result.protocol !== "mongodb:" && result.protocol !== "mongodb+srv:") {
    return callback(
      new Error("Invalid schema, expected `mongodb` or `mongodb+srv`")
    );
  }
  ...
  if (result.hostname.split(".").length < 3) {
    return callback(
      new Error("URI does not have hostname, domain name and tld")
    );
  }
  ...
  if (result.pathname && result.pathname.match(",")) {
    return callback(
      new Error("Invalid URI, cannot contain multiple hostnames")
    );
  }
  ...
  dns.resolveSrv(srvAddress, function(err, addresses) {
    ...
    for (let i = 0; i < addresses.length; i++) {
      if (
        ...
        return callback(
          new Error("Server record does not share hostname with parent URI")
        );
      }
    }
    ...
    dns.resolveTxt(result.host, function(err, record) {
      ...
      if (record) {
        ...
        if (!record.includes("authSource") && !record.includes("replicaSet")) {
          return callback(
            new Error("Text record must only set `authSource` or `replicaSet`")
          );
        }
...
function parseConnectionString(url, options) {
  ...
  if (result.query) {
    for (let name in result.query) {
      ...
      if (result.query[name] === "") {
        throw new Error(
          "Query parameter " + name + " is an incomplete value pair"
        );
      }
    }
  }
  ...
  if (connection_part.indexOf(".sock") !== -1) {
    if (connection_part.indexOf(".sock/") !== -1) {
      ...
      if (dbName.indexOf("/") !== -1) {
        if (
          dbName.split("/").length === 2 &&
          dbName.split("/")[1].length === 0
        ) {
