...
function mergeBatchResults(ordered, batch, bulkResult, err, result) {
  ...
  if (result.opTime || result.lastOp) {
    ...
    if (opTime && opTime._bsontype === "Timestamp") {
      ...
      // Current OpTime TS
      const opTimeTS =
        typeof opTime.ts === "number" ? Long.fromNumber(opTime.ts) : opTime.ts;
      const opTimeT =
        typeof opTime.t === "number" ? Long.fromNumber(opTime.t) : opTime.t;
...
class FindOperators {
  ...
  update(updateDocument) {
    // Perform upsert
    const upsert =
      typeof this.s.currentOp.upsert === "boolean"
        ? this.s.currentOp.upsert
        : false;
...
class BulkOperationBase {
  ...
  constructor(topology, collection, options, isOrdered) {
    ...
    const maxBatchSizeBytes =
      isMaster && isMaster.maxBsonObjectSize
        ? isMaster.maxBsonObjectSize
        : 1024 * 1024 * 16;
    const maxWriteBatchSize =
      isMaster && isMaster.maxWriteBatchSize
    ...
    this.s = {
      ...
      // check keys
      checkKeys:
        typeof options.checkKeys === "boolean" ? options.checkKeys : true
    };
    ...
    if (finalOptions.retryWrites) {
      if (config.batch.batchType === UPDATE) {
        finalOptions.retryWrites =
          finalOptions.retryWrites &&
          !config.batch.operations.some(op => op.multi);
      }

      if (config.batch.batchType === REMOVE) {
        finalOptions.retryWrites =
          finalOptions.retryWrites &&
          !config.batch.operations.some(op => op.limit === 0);
      }
    }
  ...
  handleWriteError(callback, writeResult) {
    if (this.s.bulkResult.writeErrors.length > 0) {
      ...
      handleCallback(
        callback,
        new BulkWriteError(
          toError(writeResult.getWriteConcernError()),
          writeResult
        ),
        null
      );
