...
const handleMongoWriteConcernError = require("./common")
...
function addToOperationsList(bulkOperation, docType, document) {
  ...
  if (bulkOperation.s.currentBatch == null)
    bulkOperation.s.currentBatch = new Batch(
      docType,
      bulkOperation.s.currentIndex
    );

  // Check if we need to create a new batch
  if (
    bulkOperation.s.currentBatch.size + 1 >=
      bulkOperation.s.maxWriteBatchSize ||
  ...
  bulkOperation.s.currentBatch.originalIndexes.push(
    bulkOperation.s.currentIndex
  );
...
class UnorderedBulkOperation extends BulkOperationBase {
  ...
  execute(_writeConcern, options, callback) {
    ...
    return executeOperation(this.s.topology, executeBatches, [
      this,
      options,
      callback
    ]);
...
function executeBatch(bulkOperation, batch, options, callback) {
  function resultHandler(err, result) {
    ...
    if (
      ((err && err.driver) || (err && err.message)) &&
...
function executeBatches(bulkOperation, options, callback) {
  ...
  for (let i = 0; i < bulkOperation.s.batches.length; i++) {
    executeBatch(bulkOperation, bulkOperation.s.batches[i], options, function(
      err
    ) {
      if (hasErrored) {
        return;
