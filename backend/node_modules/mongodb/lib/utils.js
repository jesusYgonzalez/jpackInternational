...
const executeOperation = (topology, operation, args, options) => {
  ...
  if (args[args.length - 1] != null) {
    throw new TypeError(
      "final argument to `executeOperation` must be a callback"
    );
  }
...
function applyWriteConcern(target, sources, options) {
  ...
  if (
    coll &&
    (coll.writeConcern.w != null ||
      coll.writeConcern.j != null ||
      coll.writeConcern.fsync != null)
  ) {
...
function resolveReadPreference(options, sources) {
  ...
  if (options.readPreference) {
    ...
    session.transaction.options.readPreference
  ) {
...
const emitDeprecationWarning = process.emitWarning
  ? emitProcessWarning
  : emitConsoleWarning;
...
function deprecateOptions(config, fn) {
  ...
  function deprecated() {
    ...
    config.deprecatedOptions.forEach(deprecatedOption => {
      if (
        options.hasOwnProperty(deprecatedOption) &&
